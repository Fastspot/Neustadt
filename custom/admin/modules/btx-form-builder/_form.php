<section>
	<?
		...
		if ($action_title == "Add Form") {
			$existing_forms = $fb->getAll("title ASC");
			if (count($existing_forms)) {
	?>
	<div class="alert">
		<label>Form Template</label>
		<select name="existing" id="form_builder_existing_form">
			<option value="<?=$admin_root?>btx-form-builder/add/">-- New Form --</option>
			<? foreach ($existing_forms as $item) { ?>
			<option value="<?=$admin_root?>btx-form-builder/add/?template=<?=$item["id"]?>"<? if ($form["id"] == $item["id"]) { ?> selected="selected"<? } ?>><?=$item["title"]?></option>
			<? } ?>
		</select>
	</div>
	<?
	...
	<? if (isset($settings["accept_payments"]) && $settings["accept_payments"]) { ?>
	<fieldset>
		...
		<?
			...
			foreach ($form["fields"] as $field) {
				...
				if ($t != "column") {
					...
					echo '<div class="form_builder_element form_builder_'.$t.'" id="form_builder_element_'.$count.'">';
					include BigTree::path("admin/ajax/btx-form-builder/populate-element.php");
					echo '</div>';
				} else {
					if (!$in_cwrapper) {
					...
					foreach ($field["fields"] as $subfield) {
						...
						echo '<div class="form_builder_element form_builder_'.$subfield["type"].'" id="form_builder_element_'.$count.'">';
						include BigTree::path("admin/ajax/btx-form-builder/populate-element.php");
						echo '</div>';
					}
					$count++;
