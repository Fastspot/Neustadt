...
<fieldset>
	...
	<div class="form_builder_wrap">
		<div class="form_builder_card_type">
			<div class="form_builder_select_wrapper">
				<select name="<?=$field_name?>[type]" id="form_builder_field_<?=$count?>">
					<? foreach ($card_types as $type => $desc) { ?>
					<option value="<?=$type?>"<? if ($type == $default["type"]) { ?> selected="selected"<? } ?>><?=$desc?></option>
					<? } ?>
				</select>
			</div>
			<label for="form_builder_field_<?=$count?>" class="form_builder_sublabel">Type</label>
		</div>
		<? $count++; ?>
