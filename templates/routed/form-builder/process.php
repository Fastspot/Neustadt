<?
	...
	foreach ($form["fields"] as $field) {
		...
		// If it is a column, we need to process everything inside the column first.
			...
			foreach ($field["fields"] as $subfield) {
				...
				$value = false;
				include "process-types/".$subfield["type"].".php";
				if ($value !== false) {
	...
	if ($form["paid"]) {
		...
		$transaction = $pg->charge(round($total,2),0,implode(" ",$_POST["fb_cc_billing_name"]),$_POST["fb_cc_card"]["number"],$_POST["fb_cc_card"]["month"].$_POST["fb_cc_card"]["year"],$_POST["fb_cc_card"]["code"],$_POST["fb_cc_billing_address"],$page_header);
		if (!$transaction) {
			unset($_POST["fb_cc_card"]["number"]);
			unset($_POST["fb_cc_card"]["code"]);
			$_SESSION["form_builder"]["fields"] = $_POST;
	...
	sqlquery("INSERT INTO btx_form_builder_entries (`form`,`data`,`created_at`) VALUES ('".$form["id"]."','".sqlescape(json_encode($entry))."',NOW())");
	...
	$email_title = $page_header ? $page_header : $page["nav_title"];
...
'.$email;
	...
	header("Location: ".$www_root.$page["path"]."/thanks/");
	die();
?>