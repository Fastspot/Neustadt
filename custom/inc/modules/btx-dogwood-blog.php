<?
	...
	*/
	...
	class BTXDogwood extends BigTreeModule {		
		...
		var $AuthorFilter = "author != 2";
		
		function __construct($marks = false) {
			if ($marks) {
				$this->AuthorFilter = "author = 2";
			}
		}
		...
		/*
			...
			Returns:
				Comment ID if comment was added, otherwise false
		*/
			
		function addComment($post, $parent, $name, $email, $comment, $ip = "") {
			if (!$post || !$name || !$email || !$comment) {
				return false;
			}
			
			$keys = array("post","parent","name","email","comment","ip","date");
			$vals = array($post,$parent,$name,$email,$comment,$ip,date("Y-m-d H:i:s"));
			
			sqlquery("INSERT INTO btx_dogwood_comments (`" . implode("`,`", $keys) . "`) VALUES ('" . implode("','", $vals) . "')");
			$id = sqlid();
			
			$am = new BigTreeAutoModule;
			$am->cacheNewItem($id,"btx_dogwood_comments");
		...
		*/
		...
		function getCategories($sort = "position DESC, id ASC") {
			...
			$q = sqlquery("SELECT DISTINCT cat.* FROM btx_dogwood_categories AS cat, btx_dogwood_post_categories AS rel, btx_dogwood_posts AS post WHERE post.id = rel.post AND rel.category = cat.id AND post." . $this->AuthorFilter . " AND (post.publish_date IS NULL OR post.publish_date <= '".date("Y-m-d")."') ORDER BY cat.$sort");
			while ($f = sqlfetch($q)) {
				$items[] = $f;
			}
			return $items;
		...
		function getCategoriesByPost($post){
			$return = array();
			$q = sqlquery("SELECT category.* FROM btx_dogwood_categories AS category, btx_dogwood_post_categories AS rel WHERE rel.post = ".$post." AND category.id = rel.category ORDER BY category.title ASC");
			while ($f = sqlfetch($q)) {
				$return[] = $this->get($f);
			}
			return $return;
		...
		*/
		...
		/*
			Function: getCommentCountForPost
				Returns comment count for a post
			
			Parameters:
				post - Post ID or post array.
			
			Returns:
				A number of comments
		*/
			
		function getCommentCountForPost($post) {
			if (is_array($post)) {
				$post = $post["id"];
			}
			return sqlrows(sqlquery("SELECT id FROM btx_dogwood_comments WHERE post = '".$post."' AND approved = 'on'"));
		}
		
		/*
			Function: getCommentsByParent
				Returns comment replies to a parent comment
			
			Parameters:
				parent - Comment ID.
			
			Returns:
				An array of comment replies.
		*/
			
		function getCommentsByParent($parent) {
			$comments = array();
			$q = sqlquery("SELECT * FROM btx_dogwood_comments WHERE parent = '".$parent."' AND approved = 'on' ORDER BY date ASC");
			while ($f = sqlfetch($q)) {
				$f["replies"] = $this->getCommentsByParent($f["id"]);
				$comments[] = $f;
			}
			return $comments;
		}
		...
		/*
			Function: getCommentsForPost
				Returns threaded comments for a post
			
			Parameters:
				post - Post ID or post array.
			
			Returns:
				A threaded array of comments.
		*/
		...
		function getCommentsForPost($post) {
			...
			$comments = array();
			$q = sqlquery("SELECT * FROM btx_dogwood_comments WHERE post = '".$post."' AND parent = '0' AND approved = 'on' ORDER BY date ASC");
			while ($f = sqlfetch($q)) {
				$f["replies"] = $this->getCommentsByParent($f["id"]);
				$comments[] = $f;
			}
		...
		*/
		...
		function getPageOfPosts($page,$per_page = 5) {
			...
			$q = sqlquery("SELECT * FROM btx_dogwood_posts WHERE (publish_date IS NULL OR publish_date <= '".date("Y-m-d")."') AND ".$this->AuthorFilter." ORDER BY date DESC LIMIT $start,$per_page");
			while ($f = sqlfetch($q)) {
				$posts[] = $this->getPost($f);
			}
			return $posts;
		...
		*/
		...
		function getPostCount() {
			$f = sqlfetch(sqlquery("SELECT COUNT(*) AS `count` FROM btx_dogwood_posts WHERE (publish_date IS NULL OR publish_date <= '".date("Y-m-d")."') AND ".$this->AuthorFilter));
			return $f["count"];
		}
